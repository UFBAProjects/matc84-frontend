...
} from '../interfaces-api/src/usuario/IUsuario.model';
...
export class TelaCadastroUsuarioComponent implements OnInit {
  ...
  constructor(
    ...
    private emitirEvento: EmitirEventoService
  ) {}
        ...
        this.loginDeUsuario({
          ...
          .then((response) => {
            ...
            localStorage.setItem(
              'usuario',
              btoa(JSON.stringify(response.usuario))
            );
  ...
  async loginDeUsuario(
    usuario: ILoginUsuarioRequest
  ): Promise<ILoginUsuarioResponse> {
